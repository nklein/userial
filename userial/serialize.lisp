...
(defmethod serialize ((type (eql :bytes)) value &optional (buffer *buffer*))
  ...
  (labels ((add-bytes (value start end buffer)
	     (let* ((length    (- end start))
		    (buf-start (buffer-length buffer)))
	       (buffer-advance length buffer)
	       (setf (subseq buffer buf-start (buffer-length buffer))
...
(defmethod unserialize ((type (eql :bytes)) &optional (buffer *buffer*))
  ...
  (labels ((add-segment (buffer start end output)
	     (let* ((length (- end start))
		    (out-start (buffer-length output)))
	       (buffer-advance length output)
	       (setf (subseq output out-start (length output))
    ...
    (let ((output (make-buffer))
	  (start (buffer-length buffer)))
      ;; temporarily set buffer length to whole capacity
      ;; then, set buffer length to be the return of #'do-segment
      (setf (buffer-length buffer) (buffer-capacity buffer))
      (setf (buffer-length buffer) (do-segment buffer start output))
